<!--
  Estimator Database Frontend -- data to be coneected to SQL database
  ```
  ```src="https://pgba.com/images/pgba-oneline.svg" alt="PGBA">
-->
<template>
    <!--
        dashboard -- nav names will change to match estimator db
      ```
    -->
    <div>
              ...
              <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-indigo-700 pt-5 pb-4">
                ...
                <div class="mt-5 h-0 flex-1 overflow-y-auto">
                  <nav class="space-y-1 px-2">
                    <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-600', 'group flex items-center rounded-md px-2 py-2 text-base font-medium']">
      ...
      <div class="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        ...
        <div class="flex flex-grow flex-col overflow-y-auto bg-indigo-700 pt-5">
          ...
          <!--TODO: FIX COLOR to Show better PGBA Logo -->
            <img class="h-8 w-auto" src="../assets/favicon.ico" alt="PGBA" />
           <!-- src="https://pgba.com/images/pgba-oneline.svg" alt="PGBA" /> -->
          </div>
          <div class="mt-5 flex flex-1 flex-col">
            <nav class="flex-1 space-y-1 px-2 pb-4">
              <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-600', 'group flex items-center rounded-md px-2 py-2 text-sm font-medium']">
              ...
              <Menu as="div" class="relative ml-3">
                <div>
                  <MenuButton class="flex max-w-xs items-center rounded-full bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                    ...
                    <!-- TODO: user profile -->
                    <img class="h-8 w-8 rounded-full" src="../assets/PROFILE.png" alt="" />
